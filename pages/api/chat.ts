...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const openai = new OpenAI({
    apiKey: "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",
  });
...
console.log(conversationHistory);
    const prompt = `You are Gandalf from the Lord of the Rings and this is the conversation you have been having with the user so far:\n(((${formattedHistory})))\nDon't greet the user after every message if a history already exists. Using this history, respond to this message:\n${userMessage}\n`;
    ...
    conversationHistory.push(gandalfReply);
